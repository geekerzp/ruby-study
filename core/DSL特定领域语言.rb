...
class XML
  ...
  def tag(tagname, attributes={})
    ...
    attributes.each {|attr,value| @out << " #{attr}='#{value}'" } unless attributes.empty?
  ...
  # The code below is what chagnes this from an ordinary class into a DSL
...
class XMLGrammar
  ...
  # hold the output. This can be any object that responds to <<(String).
  def initialize(out)
    @out = out    # Remember wher to send out output 
  end 
  ...
  # Define an allowed element (or tag) in this grammar.
  ...
  # These are contants used when defining attribute values.
  ...
  def tag(tagname, attributes={})
    ...
    # Assuming they are allowed, output all specified ones.
    ...
    # required attributes that were moitted and for attributes with 
    ...
    nil   # Tags output themselves, so them don't return any content.
  end
end 
